...
-- 4c. Oh, no! The actor HARPO WILLIAMS was accidentally entered in the actor table as GROUCHO WILLIAMS, the name of Harpos second cousins husbands yoga teacher. Write a query to fix the record.
UPDATE actor 
...
-- of the actor is currently HARPO, change it to GROUCHO. Otherwise, change the first name to 
...
SELECT s.staff_id, SUM(p.amount) as "Total Payments"
...
SELECT COUNT(i.film_id) AS "Count"
FROM inventory AS i INNER JOIN film AS f
	ON i.film_id = f.film_id
WHERE f.title = "Hunchback Impossible";

-- 6e. Using the tables payment and customer and the JOIN command, list the total paid by each customer. List the customers alphabetically by last name:
SELECT c.customer_id, c.last_name, SUM(p.amount) AS "Total Payment"
...
SELECT f.title, COUNT(r.rental_id) AS 'Rentals' FROM
...
SELECT COUNT(payment.amount), store.store_id FROM
store INNER JOIN payment 
	ON store.manager_staff_id = payment.staff_id
GROUP BY store.store_id;

-- 7g. Write a query to display for each store its store ID, city, and country.
SELECT s.store_id, ct.city, cr.country FROM
...
-- payment, and rental.)
SELECT category.name, SUM(payment.amount) as 'Total' FROM
